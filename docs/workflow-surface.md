...
9. Use **Coach_me Guided Reasoning Mode** for broad, messy, strategic, document-dependent, or customer-situation questions. Insurance Copilot uses the standalone **coach-me** skill for dynamic questioning: check the **source discovery order**, ask one question at a time in an interactive conversational gateway, provide a **recommended default answer** when useful, offer **answer now or continue questioning**, **automatically stop questioning when information is sufficient**, keep a **Coach_me Working Document**, treat **Q&A intake is raw source input**, respect **public institution knowledge**, **agent-private workspace**, and **customer-specific materials**, and then route back to Insurance Copilot for source grounding, review gates, and no-external-action boundaries. Perform **no automatic persistence**. This remains a **manual-first practitioner workflow**.
...
- **When to use:** The agent asks a broad, messy, strategic, document-dependent, customer-situation, or **product recommendation intent** question where a one-shot answer may miss material facts.
- **Routing:** Use the standalone **coach-me** skill before Insurance Copilot's domain workflow. Use **Coach_me before Client Needs Intake** when the agent asks how to recommend, judge, or handle insurance/product recommendation.
- **Method posture:** Coach_me is a questioning-to-document method: **question → obtain information → form a working document → recommend next route**. Insurance Copilot adds source hierarchy, public/private boundaries, customer-first advocacy, and review gates.
- **Required inputs:** One natural-language question is enough to start. Use available sources before asking.
...
- **Question rule:** Use dynamic questioning, not a fixed questionnaire. Ask the next most useful question, one question at a time in any **interactive conversational gateway**, with why it matters and a **recommended default answer** when useful. Do not force exactly three questions and do not force Direction/Risk/Source/Action categories. Offer **answer now or continue questioning** when useful and **automatically stop questioning when information is sufficient**.
- **Output:** **Coach_me Working Document** and, when ready, an insurance handoff using `skills/insurance_copilot/templates/coach-me.md`. Treat **Q&A intake is raw source input**.
...
```text
Use the standalone coach-me method first. Start a Coach_me Working Document, check source discovery order before asking, ask the next most useful dynamic question one at a time with a recommended default answer when useful, and stop when information is sufficient. Then route back to Insurance Copilot for the appropriate insurance workflow, source grounding, Professional Review Gate, and no-external-action boundary.
```
