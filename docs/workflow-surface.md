...
- **Question rule:** Compute an **information sufficiency score**, then apply the **three-question decision algorithm** with **Direction / Risk / Source / Action**: ask exactly three most precise and relevant questions as **one direction question, one risk question, one action/source question**. In Telegram/chat, ask **one question at a time** (`Question 1/3`, wait, `Question 2/3`, wait, `Question 3/3`, wait); do not batch all three questions unless the agent asks for an offline checklist. After `Question 3/3`, offer **answer now or continue questioning**. State that Coach_me will **automatically stop questioning when information is sufficient**.
