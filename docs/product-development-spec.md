...
Current guided reasoning runtime method: standalone **coach_me** provides the generic questioning-to-document loop, while Insurance Copilot provides the domain adapter through `skills/insurance_copilot/SKILL.md`, `references/coach_me.md`, and `templates/coach_me.md`. It covers broad, messy, strategic, document-dependent, customer-situation, and **product recommendation intent** questions, with **Coach_me before Client Needs Intake** for recommend/judge/handle-product questions. The required runtime behavior is dynamic questioning, source discovery before asking, one question at a time in interactive conversational gateways, recommended defaults for unsure agents, a **Coach_me Working Document**, and domain handoff to the appropriate Insurance Copilot workflow plus review gates. It must not regress to a fixed three-question runtime contract or fixed Direction/Risk/Source/Action categories.
...
- For Coach_me Guided Reasoning Mode, use the standalone **coach_me** method plus the Insurance Copilot adapter; route **product recommendation intent** to **Coach_me before Client Needs Intake**; follow source discovery order; ask the next most useful dynamic question one at a time in any **interactive conversational gateway**; include a **recommended default answer** when useful; offer answer now or continue questioning; automatically stop questioning when information is sufficient; keep a Coach_me Working Document; treat Q&A intake is raw source input; respect public institution knowledge, agent-private workspace, and customer-specific materials; keep no automatic persistence; do not force a fixed three-question contract or fixed Direction/Risk/Source/Action categories.
...
- stronger New Agent Coach Mode and Coach_me Guided Reasoning Mode scenario coverage, including source discovery order, dynamic one-question-at-a-time clarification, answer now or continue questioning, automatically stop questioning when information is sufficient, Coach_me Working Document, Q&A intake is raw source input, and insurance workflow handoff;
