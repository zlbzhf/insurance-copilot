...
Current guided reasoning runtime workflow: **Coach_me Guided Reasoning Mode**. It translates document-grounded questioning and grill-style clarification into one insurance-specific workflow through `skills/insurance_copilot/SKILL.md`, `references/coach-me.md`, `templates/coach-me.md`, evals, tests, and validators. It covers broad, messy, strategic, document-dependent, customer-situation, and **product recommendation intent** questions, with **Coach_me before Client Needs Intake** for recommend/judge/handle-product questions. **Coach_me v2 Productized Workflow** moves Coach_me **from questioning feature to agent workbench center**: Coach_me is **one workflow, not two skills**; it uses **source discovery order** across **public institution knowledge**, **agent-private workspace**, and **customer-specific materials**; computes an **information sufficiency score**; applies the **three-question decision algorithm** with **Direction / Risk / Source / Action**; asks exactly three most precise and relevant questions as **one direction question, one risk question, one action/source question**. In any **interactive conversational gateway**, use the **sequential question protocol**: ask **one question at a time**, **send only the active question** in the **current turn** (`Question 1/3`, `Question 2/3`, `Question 3/3`) instead of batching unless an offline checklist is requested, and include a **recommended default answer** when useful; offers **answer now or continue questioning**; will **automatically stop questioning when information is sufficient**; keeps a **Coach_me Working Document**; treats **Q&A intake is raw source input**; uses a **capability ladder** so **limitations become product states** with **default safe draft mode**, **review-ready packet**, **confirmed persistence packet**, and **external action handoff packet**; ends with a **Karpathy-style LLM wiki backfeed proposal** plus **Backfeed Decision Packet**; and performs **no automatic persistence** because **no automatic persistence is a product boundary, not a dead end**. This remains a **manual-first practitioner workflow**. Source-sensitive or regulated variants still use **Source Grounding and Data Boundary Gate** and **Professional Review Gate**.
...
- For Coach_me Guided Reasoning Mode, use one workflow, not two skills; route **product recommendation intent** to **Coach_me before Client Needs Intake**; follow source discovery order; ask exactly three most precise and relevant questions; in any **interactive conversational gateway**, use the **sequential question protocol**: ask one question at a time, **send only the active question** in the **current turn** (`Question 1/3`, `Question 2/3`, `Question 3/3`) instead of batching unless an offline checklist is requested, and include a **recommended default answer** when useful; offer answer now or continue questioning; automatically stop questioning when information is sufficient; keep a Coach_me Working Document; treat Q&A intake is raw source input; respect public institution knowledge, agent-private workspace, and customer-specific materials; propose a Karpathy-style LLM wiki backfeed proposal; keep no automatic persistence.
