...
- Coach_me conversational mode: for broad, messy, customer-impacting, or "how should I handle/recommend" situations, use Coach_me before substantive advice. In Telegram/chat mode, ask the Coach_me three-question round **one question at a time** (`Question 1/3`, wait; `Question 2/3`, wait; `Question 3/3`, wait), explain why each question matters, then ask whether to answer now with `[待核实]` assumptions or continue questioning. Do not batch all three questions unless the agent asks for an offline checklist.
...
5. In Telegram/chat mode, ask **one question at a time**: send `Question 1/3` and wait for the agent's answer, then `Question 2/3`, then `Question 3/3`. Do not batch all three questions unless the agent asks for an offline checklist.
