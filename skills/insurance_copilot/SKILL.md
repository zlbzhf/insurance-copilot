...
- Coach_me conversational mode: for broad, messy, customer-impacting, product recommendation intent, or "how should I handle/recommend" situations, route to **Coach_me before Client Needs Intake** and before substantive advice. In any **interactive conversational gateway**, use the **sequential question protocol**: ask the Coach_me three-question round **one question at a time**, send only the active question in the current turn (`Question 1/3`, wait; `Question 2/3`, wait; `Question 3/3`, wait), explain why each question matters, include a **recommended default answer** when the agent is unsure, then ask whether to answer now with `[待核实]` assumptions or continue questioning. Do not batch all three questions unless the agent asks for an offline checklist.
...
- For broad, messy, strategic, document-dependent, customer-impacting, product recommendation intent, or customer-situation questions where a one-shot answer may miss material facts, activate **Coach_me Guided Reasoning Mode** from `references/coach-me.md` and shape the working/final output with `templates/coach-me.md`; route **Coach_me before Client Needs Intake** when the agent asks how to recommend, judge, or handle a product recommendation rather than merely asking for a structured fact-find. Coach_me is **one workflow, not two skills** and now uses **Coach_me v2 Productized Workflow** to move **from questioning feature to agent workbench center**: first use the **source discovery order**, then compute an **information sufficiency score**, use the **three-question decision algorithm** with **Direction / Risk / Source / Action** — **one direction question, one risk question, one action/source question** — ask exactly three most precise and relevant questions, but in any **interactive conversational gateway** deliver the **sequential question protocol** **one question at a time** and **send only the active question** in the **current turn** (`Question 1/3` then wait, `Question 2/3` then wait, `Question 3/3` then wait). Each active question should include why it matters, a good answer format, and a **recommended default answer** for an unsure/new agent. Do not batch all three questions unless the agent asks for an offline checklist. After `Question 3/3`, offer **answer now or continue questioning**, state that you will **automatically stop questioning when information is sufficient**, treat **Q&A intake is raw source input**, and end with a **Coach_me Working Document** / final answer plus a **Karpathy-style LLM wiki backfeed proposal** and **Backfeed Decision Packet**. Use the **capability ladder** so **limitations become product states**: **default safe draft mode**, **review-ready packet**, **confirmed persistence packet**, and **external action handoff packet**. Respect **public institution knowledge**, **agent-private workspace**, and **customer-specific materials** boundaries; use **Source Grounding and Data Boundary Gate** and **Professional Review Gate** when applicable; there is **no automatic persistence**, and **no automatic persistence is a product boundary, not a dead end**. This remains a **manual-first practitioner workflow**.
...
- **Coach_me Guided Reasoning Mode:** `references/coach-me.md` with `templates/coach-me.md` for broad, messy, strategic, document-dependent, customer-impacting, product recommendation intent, or customer-situation questions; use **Coach_me before Client Needs Intake** when the agent asks how to recommend/judge/handle insurance rather than merely requesting a fact-find. It is **one workflow, not two skills** and uses **Coach_me v2 Productized Workflow** to move **from questioning feature to agent workbench center**. It uses the **source discovery order**, computes an **information sufficiency score**, applies the **three-question decision algorithm** with **Direction / Risk / Source / Action** — **one direction question, one risk question, one action/source question** — asks exactly three most precise and relevant questions, but in any **interactive conversational gateway** uses the **sequential question protocol**: ask **one question at a time**, **send only the active question** in the **current turn** (`Question 1/3`, wait, `Question 2/3`, wait, `Question 3/3`, wait), include why it matters, good answer format, and a **recommended default answer** when useful, then offer **answer now or continue questioning** after `Question 3/3`. It will **automatically stop questioning when information is sufficient**, treats **Q&A intake is raw source input**, respects **public institution knowledge**, **agent-private workspace**, and **customer-specific materials**, proposes a **Karpathy-style LLM wiki backfeed proposal** and **Backfeed Decision Packet**, uses the **capability ladder** so **limitations become product states** with **default safe draft mode**, **review-ready packet**, **confirmed persistence packet**, and **external action handoff packet**, and performs **no automatic persistence** because **no automatic persistence is a product boundary, not a dead end**. Do not batch all three questions unless the agent asks for an offline checklist. This remains a **manual-first practitioner workflow**.
...
Use **Coach_me Guided Reasoning Mode** when the agent asks a broad, messy, strategic, document-dependent, product recommendation intent, or customer-situation question where a one-shot answer may miss material facts. If the agent asks “how should I recommend / what should I recommend / 怎么推荐保险 / 推荐保险产品” without enough facts, route **Coach_me before Client Needs Intake**: first clarify direction, risk, and action/source, then let Client Needs Intake collect the structured fact-find if needed. Coach_me is **one workflow, not two skills** and uses `references/coach-me.md` plus `templates/coach-me.md`.

**Coach_me v2 Productized Workflow** moves Coach_me **from questioning feature to agent workbench center**. It keeps the earlier follow-up rules but adds a **capability ladder**, **information sufficiency score**, **Direction / Risk / Source / Action** framing, and **Backfeed Decision Packet** so **limitations become product states**. **no automatic persistence is a product boundary, not a dead end**. This remains a **manual-first practitioner workflow**.
...
3. Compute an **information sufficiency score** for Direction, Risk, Source, and Action.
4. Use the **three-question decision algorithm**: one round asks exactly three most precise and relevant questions as **one direction question, one risk question, one action/source question**.
5. In any **interactive conversational gateway**, use the **sequential question protocol**: ask **one question at a time**, **send only the active question** in the **current turn**, include why it matters, good answer format, and a **recommended default answer** when useful; send `Question 1/3` and wait for the agent's answer, then `Question 2/3`, then `Question 3/3`. Do not batch all three questions unless the agent asks for an offline checklist.
6. After `Question 3/3`, offer **answer now or continue questioning**.
7. State that you will **automatically stop questioning when information is sufficient**; the user may also stop and ask for a current answer.
8. Treat **Q&A intake is raw source input**.
9. Produce a final answer document plus **Karpathy-style LLM wiki backfeed proposal** and **Backfeed Decision Packet**.
10. Use the **capability ladder** to choose **default safe draft mode**, **review-ready packet**, **confirmed persistence packet**, or **external action handoff packet**.
11. Keep **no automatic persistence**: do not write customer/private facts or public-pack updates unless the user explicitly approves destination and scope.
12. Apply **Source Grounding and Data Boundary Gate** and **Professional Review Gate** when source status, public/private data, customer-facing use, regulated use, or side-effect adjacency requires them.
...
- [ ] Coach_me Guided Reasoning Mode used for broad, messy, strategic, document-dependent, customer-impacting, product recommendation intent, or customer-situation questions; verify **Coach_me before Client Needs Intake** for recommend/judge/handle-product questions; verify one workflow, not two skills; **Coach_me v2 Productized Workflow**; **from questioning feature to agent workbench center**; source discovery order; **information sufficiency score**; **Direction / Risk / Source / Action**; **three-question decision algorithm**; ask exactly three most precise and relevant questions as **one direction question, one risk question, one action/source question**; in any **interactive conversational gateway** use the **sequential question protocol**: ask **one question at a time**, **send only the active question** in the **current turn**, include a **recommended default answer** when useful, and do not batch all three questions unless offline checklist is requested; `Question 1/3`, `Question 2/3`, `Question 3/3`; answer now or continue questioning; automatically stop questioning when information is sufficient; Coach_me Working Document; **capability ladder** with **default safe draft mode**, **review-ready packet**, **confirmed persistence packet**, and **external action handoff packet**; public institution knowledge / agent-private workspace / customer-specific materials boundaries; Q&A intake is raw source input; Karpathy-style LLM wiki backfeed proposal; **Backfeed Decision Packet**; **limitations become product states**; no automatic persistence; **no automatic persistence is a product boundary, not a dead end**; **manual-first practitioner workflow**.
