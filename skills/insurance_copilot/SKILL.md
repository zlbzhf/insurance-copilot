...
- **Coach_me Guided Reasoning Mode:** `references/coach-me.md` with `templates/coach-me.md` for broad, messy, strategic, document-dependent, or customer-situation questions; it is **one workflow, not two skills** and uses **Coach_me v2 Productized Workflow** to move **from questioning feature to agent workbench center**. It uses the **source discovery order**, computes an **information sufficiency score**, applies the **three-question decision algorithm** with **Direction / Risk / Source / Action** — **one direction question, one risk question, one action/source question** — asks exactly three most precise and relevant questions, offers **answer now or continue questioning**, will **automatically stop questioning when information is sufficient**, treats **Q&A intake is raw source input**, respects **public institution knowledge**, **agent-private workspace**, and **customer-specific materials**, proposes a **Karpathy-style LLM wiki backfeed proposal** and **Backfeed Decision Packet**, uses the **capability ladder** so **limitations become product states** with **default safe draft mode**, **review-ready packet**, **confirmed persistence packet**, and **external action handoff packet**, and performs **no automatic persistence** because **no automatic persistence is a product boundary, not a dead end**. This remains a **manual-first practitioner workflow**.
...
4. Use the **three-question decision algorithm**: ask exactly three most precise and relevant questions as **one direction question, one risk question, one action/source question**.
5. Offer **answer now or continue questioning**.
6. State that you will **automatically stop questioning when information is sufficient**; the user may also stop and ask for a current answer.
7. Treat **Q&A intake is raw source input**.
8. Produce a final answer document plus **Karpathy-style LLM wiki backfeed proposal** and **Backfeed Decision Packet**.
9. Use the **capability ladder** to choose **default safe draft mode**, **review-ready packet**, **confirmed persistence packet**, or **external action handoff packet**.
10. Keep **no automatic persistence**: do not write customer/private facts or public-pack updates unless the user explicitly approves destination and scope.
11. Apply **Source Grounding and Data Boundary Gate** and **Professional Review Gate** when source status, public/private data, customer-facing use, regulated use, or side-effect adjacency requires them.
