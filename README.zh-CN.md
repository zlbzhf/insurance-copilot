...
对于宽泛、凌乱、策略性、依赖资料、客户场景或 **product recommendation intent**（例如“怎么推荐保险 / 推荐保险产品”）的问题，先使用独立的 **coach-me** skill，再回到 Insurance Copilot 做保险领域路由和复核边界。当代理人是在问如何推荐、判断或处理保险/产品推荐时，先走 **Coach_me before Client Needs Intake**。Coach_me 是动态的“追问→获得信息→形成文档→推荐路由”方法：先检查 **source discovery order**，在 **interactive conversational gateway** 中一次只问一个最有价值的问题，必要时给出 **recommended default answer**，持续更新 **Coach_me Working Document**，提供 **answer now or continue questioning** 选择，并在 **automatically stop questioning when information is sufficient** 时停止追问。它不是固定问卷、不是固定问题数量，也不强制 Direction/Risk/Source/Action 分类。Insurance Copilot 随后区分 **public institution knowledge**、**agent-private workspace** 与 **customer-specific materials**，把 **Q&A intake is raw source input**，必要时套用 **Source Grounding and Data Boundary Gate** / **Professional Review Gate**，并保持 **no automatic persistence** 和 **manual-first practitioner workflow**。
