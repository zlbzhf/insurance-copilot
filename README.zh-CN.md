...
对于宽泛、凌乱、策略性、依赖资料、客户场景或 **product recommendation intent**（例如“怎么推荐保险 / 推荐保险产品”）的问题，使用 **Coach_me Guided Reasoning Mode**（`skills/insurance_copilot/references/coach-me.md` 和 `skills/insurance_copilot/templates/coach-me.md`）。当代理人是在问如何推荐、判断或处理保险/产品推荐时，先走 **Coach_me before Client Needs Intake**，方向、风险、来源/行动明确后再进入 Client Needs Intake。Coach_me 是 **one workflow, not two skills**，并通过 **Coach_me v2 Productized Workflow** 从 **from questioning feature to agent workbench center** 升级：先检查 **source discovery order**，再计算 **information sufficiency score**，用 **three-question decision algorithm** 和 **Direction / Risk / Source / Action** 组织追问，即 **one direction question, one risk question, one action/source question**；继续 ask exactly three most precise and relevant questions。在任何 **interactive conversational gateway** 中使用 **sequential question protocol**：按 **one question at a time** 执行，**send only the active question** in the **current turn**（`Question 1/3` 等回答、`Question 2/3` 等回答、`Question 3/3` 等回答），必要时给出 **recommended default answer**；除非代理人要 offline checklist，否则不要一次性发送三个问题；提供 **answer now or continue questioning** 选择，并在 **automatically stop questioning when information is sufficient** 时停止追问。它维护 **Coach_me Working Document**，区分 **public institution knowledge**、**agent-private workspace** 与 **customer-specific materials**，把 **Q&A intake is raw source input**，最终给出 **Karpathy-style LLM wiki backfeed proposal** 与 **Backfeed Decision Packet**；同时用 **capability ladder** 让 **limitations become product states**，包括 **default safe draft mode**、**review-ready packet**、**confirmed persistence packet** 和 **external action handoff packet**。必要时套用 **Source Grounding and Data Boundary Gate** / **Professional Review Gate**，并保持 **no automatic persistence**，因为 **no automatic persistence is a product boundary, not a dead end**。它仍然是 **manual-first practitioner workflow**。
