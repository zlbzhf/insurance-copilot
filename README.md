...
For broad, messy, strategic, document-dependent, customer-situation, or **product recommendation intent** questions, use the standalone **coach_me** skill first, then return to Insurance Copilot for domain routing and review gates. Use **Coach_me before Client Needs Intake** when the agent asks how to recommend, judge, or handle insurance/product recommendation. Coach_me is a dynamic questioning-to-document method: it checks the **source discovery order**, asks the next most useful question one at a time in an **interactive conversational gateway**, includes a **recommended default answer** when useful, updates a **Coach_me Working Document**, offers **answer now or continue questioning**, and **automatically stops questioning when information is sufficient**. It is not a fixed questionnaire, not a fixed question count, and not fixed Direction/Risk/Source/Action categories. Insurance Copilot then respects **public institution knowledge**, **agent-private workspace**, and **customer-specific materials**, treats **Q&A intake is raw source input**, applies **Source Grounding and Data Boundary Gate** / **Professional Review Gate**, and keeps **no automatic persistence** in a **manual-first practitioner workflow**.
